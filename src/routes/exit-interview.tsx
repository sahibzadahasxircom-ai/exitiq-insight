...
function ExitInterview() {
  ...
  const [step, setStep] = useState(1); // next AI question index
  ...
  useEffect(() => {
    // Send the first real question after the greeting
    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <header className="border-b border-border bg-background/80 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-3xl items-center justify-between px-4">
          ...
          <span className="text-xs text-muted-foreground">Exit interview</span>
        </div>
      </header>
      ...
      {!done && (
        <div className="border-t border-border bg-background">
          <div className="mx-auto max-w-3xl px-4 py-4">
            <div className="flex items-end gap-2 rounded-xl border border-border bg-card p-2 shadow-soft focus-within:border-primary/50 focus-within:ring-2 focus-within:ring-primary/15">
              <textarea
                ref={inputRef}
                value={input}
                ...
                disabled={typing}
                className="max-h-40 min-h-[40px] w-full resize-none bg-transparent px-2 py-2 text-sm outline-none placeholder:text-muted-foreground disabled:opacity-60"
              />
              <Button
                size="icon"
                onClick={handleSend}
...
function MessageBubble({ message }: { message: Message }) {
  ...
  return (
    <div className={"flex animate-fade-in " + (isAI ? "justify-start" : "justify-end")}>
      <div className={"flex max-w-[80%] gap-3 " + (isAI ? "" : "flex-row-reverse")}>
        {isAI && (
          <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-soft">
            <span className="text-[10px] font-bold">EQ</span>
          </div>
        )}
        <div
          className={
            isAI
              ...
              : "rounded-2xl rounded-tr-sm bg-primary px-4 py-3 text-sm leading-relaxed text-primary-foreground shadow-soft"
          }
        >
          {message.text}
        </div>
  ...
  return (
    <div className="flex animate-fade-in justify-start">
      <div className="flex max-w-[80%] gap-3">
        ...
        <div className="flex items-center gap-1.5 rounded-2xl rounded-tl-sm border border-border bg-card px-4 py-3 shadow-soft">
          <Dot delay="0ms" />
          <Dot delay="150ms" />
          <Dot delay="300ms" />
        </div>
